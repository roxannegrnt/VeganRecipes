...
class DbConnect {
    ...
    public function __construct() {
        if ($this->dbb === null) {
            try {
                ...
                $this->ps_modifyRecipe = $this->dbb->prepare("UPDATE recettes,types SET `Titre`=:title,`Ingredient`=:ingredients,`Description`=:descrip,`Valider`=0,`NomFichierImg`=:img,`IdUtilisateur`=:id,"
                        . "recettes.IdType= types.IdType WHERE IdRecette=:idR AND NomType= :type");
                $this->ps_modifyRecipe->setFetchMode(PDO::FETCH_ASSOC);
    ...
    /**
     * Modifies a recipe existing in database
     * @param array $param all new parameters to add to recipe
     * @param int $id identifiaction of user
     * @param string $img name of image file of new recipe
     */
    function ModifyRecipe($param, $id, $img,$idR) {
        $this->dbb->beginTransaction();
        $this->ps_modifyRecipe->bindParam(':title', $param[0], PDO::PARAM_STR);
        $this->ps_modifyRecipe->bindParam(':ingredients', $param[1], PDO::PARAM_STR);
        $this->ps_modifyRecipe->bindParam(':descrip', $param[2], PDO::PARAM_STR);
        $this->ps_modifyRecipe->bindParam(':type', $param[3], PDO::PARAM_STR);
        $this->ps_modifyRecipe->bindParam(':img', $img, PDO::PARAM_STR);
        $this->ps_modifyRecipe->bindParam(':id', $id, PDO::PARAM_INT);
        $this->ps_modifyRecipe->bindParam(':idR', $idR, PDO::PARAM_INT);
        if ($this->ps_modifyRecipe->execute()) {
            $this->dbb->commit();
        } else {
            $this->dbb->rollBack();
        }
